...
class DatasetHandler:
    ...
    def get_fold_indices(mode, dataset_info: dict) -> List[int]:
        ...
        if mode == RunMode.TRAIN:
            ...
            fold_indices.remove(dataset_info[DatasetInfoKey.VALIDATE_FOLD][0])
            fold_indices.remove(dataset_info[DatasetInfoKey.TEST_FOLD][0])
        elif mode == RunMode.VALIDATE:
            fold_indices = dataset_info[DatasetInfoKey.VALIDATE_FOLD]
        elif mode == RunMode.TEST:
            fold_indices = dataset_info[DatasetInfoKey.TEST_FOLD]
        else:
