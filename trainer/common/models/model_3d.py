...
class I3D(torch.nn.Module):
    def __init__(
        ...
        if pre_trained:
            # set final layer to original 400 classes
            ...
            # load model
            self.load_state_dict(torch.load(pre_trained_model_path))
    ...
    def train(
        self, mode=True
    ):  # adjusted from https://discuss.pytorch.org/t/how-to-train-with-frozen-batchnorm/12106/9
        """
...
def load_conv3d(state_dict, name_pt, sess, name_tf, bias=False, bn=True):
    ...
    conv_weights_rs = np.transpose(conv_weights, (4, 3, 0, 1, 2))  # to pt format (out_c, in_c, depth, height, width)
