...
from trainer.common.constants import LATENT_KEY
...
class Model(nn.Module):
    def __init__(
        ...
        self.embedding_mm = nn.Sequential(
            nn.Linear(39, 128),
            nn.LayerNorm(128),
            nn.GELU(),
    ...
    def forward(self, x, x_mm):
        ...
        latent = self.classifier(encoders_features)[LATENT_KEY]  # (B, 336)
        latent_mm = self.embedding_mm(x_mm)  # (B, 64)
        latent_concat = torch.cat([latent, latent_mm], dim=1)

        logit = self.classifier_mm(latent_concat)
