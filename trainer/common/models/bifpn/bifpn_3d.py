...
Reference:
https://github.com/bonlime/pytorch-tools/blob/master/pytorch_tools/modules/bifpn.py
"""
...
from trainer.common.models.modules.activations import activation_from_name
from trainer.common.models.modules.blocks import DepthwiseSeparableConv, conv1x1x1
...
class FirstBiFPNLayer(BiFPNLayer):
    def __init__(self, encoder_channels, channels=64, norm_layer=ABN, norm_act="relu"):
        ...
        for enc_in_channel in encoder_channels:
            ...
            if enc_in_channel != channels:
                layer = nn.Sequential(
                    conv1x1x1(enc_in_channel, channels),
    ...
    def forward(self, features):
        ...
        for idx in range(self.num_features - 1):
            p_up.append(
                self.p_up_convs[idx](  # fuse: input and upsampled previous feature map
                    self.fuse_up[idx](features_in_1[idx + 1], self.up(p_up[-1]))
                )
            )
