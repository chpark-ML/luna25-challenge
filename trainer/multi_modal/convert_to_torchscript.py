...
def main() -> None:
    ...
    for prefix in list_prefix:
        ...
        for path_to_load_weight in path_to_load_weights:
            ...
            sample_mm = torch.rand((1, 39), dtype=torch.float32)
            ...
            for device_type, device in dict_device.items():
                ...
                sample_mm_device = sample_mm.to(device)
                ...
                with torch.no_grad():
                    # Trace the model
                    traced_model = torch.jit.trace(model_device, (sample_device, sample_mm_device))
                    ...
                    original_output = model_device(sample_device, sample_mm_device)
                    loaded_model = torch.jit.load(torchscript_path, map_location=device)
                    loaded_model.eval()
                    reloaded_output = loaded_model(sample_device, sample_mm_device)
