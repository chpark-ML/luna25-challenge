...
class CTCaseDataset(data.Dataset):
    def __init__(
        ...
        self.patch_size_large = [self.size_z_large, self.size_xy_large, self.size_xy_large]
    ...
    def __getitem__(self, idx):
        ...
        if self.fetch_from_patch:
            ...
            img = extract_patch_dicom_space(
                h5_path,
                d_coord_zyx,
                xy_size=self.size_xy_large,
                z_size=self.size_z_large,
        ...
        if self.mode_model == "2D":
            ...
            output_shape_large = (1, self.size_px_xy_large, self.size_px_xy_large)
        else:
            output_shape = (self.size_px_z, self.size_px_xy, self.size_px_xy)
            output_shape_large = (self.size_px_z_large, self.size_px_xy_large, self.size_px_xy_large)

        # Extract patches for both scales
        patch = extract_patch(
            CTData=img,
        ...
        patch_large = extract_patch(
            CTData=img,
            coord=tuple(np.array(self.patch_size_large) // 2),
            srcVoxelOrigin=origin,
            srcWorldMatrix=transform,
            srcVoxelSpacing=spacing,
            output_shape=output_shape_large,
            voxel_spacing=(
                self.size_mm / self.size_px_z_large,
                self.size_mm / self.size_px_xy_large,
                self.size_mm / self.size_px_xy_large,
            ),
            rotations=self.rotations,
            translations=translations,
        ...
        patch_large = patch_large.astype(np.float32)
