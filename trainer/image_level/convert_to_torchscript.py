...
def main() -> None:
    ...
    for prefix in list_prefix:
        ...
        for path_to_load_weight in path_to_load_weights:
            ...
            # Dummy inputs for tracing (patch_image and image_large)
            # patch_image: [B, C, D, H, W] - smaller patch
            # image_large: [B, C, D, H, W] - larger image
            patch_sample = torch.rand((1, 1, 48, 72, 72), dtype=torch.float32)  # patch input
            image_sample = torch.rand((1, 1, 64, 128, 128), dtype=torch.float32)  # image input
            ...
            for device_type, device in dict_device.items():
                ...
                patch_sample_device = patch_sample.to(device)
                image_sample_device = image_sample.to(device)
                ...
                with torch.no_grad():
                    ...
                    original_output = model_device(patch_sample_device, image_sample_device)
                    loaded_model = torch.jit.load(torchscript_path, map_location=device)
                    loaded_model.eval()
                    reloaded_output = loaded_model(patch_sample_device, image_sample_device)
