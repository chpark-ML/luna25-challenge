...
def main() -> None:
    ...
    prefix = "cls_all_model_6_val_fold"
    suffix = ""
    ...
    for prefix in list_prefix:
        ...
        for path_to_load_weight in path_to_load_weights:
            ...
            for device_type, device in dict_device.items():
                ...
                with torch.no_grad():
                    # Trace the model
                    scripted_model = torch.jit.trace(model_device, sample_device)
                ...
                elif isinstance(original_output, tuple) and isinstance(reloaded_output, tuple):
                    # NamedTuple case
                    keys = original_output._fields
                else:
                    keys = None
